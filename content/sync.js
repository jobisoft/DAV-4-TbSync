        ...
        for (let c=0; c < cards.multi.length; c++) {
            ...
            if (id !==null) {
                ...
                if (cards.multi[c].status == "200") {
                    ...
                    tbSync.errorlog.add("warning", syncdata.ownerInfo, "Unknown XML", JSON.stringify(cards.multi[c]));
                }
            }
        }
                                ...
                                if (card) {
                                    ...
                                    if (isAdding || vcard.modified) {
                                        ...
                                        if (response && response.softerror) {
                                            ...
                                            tbSync.errorlog.add("warning", syncdata.ownerInfo, "missing-permission::" + tbSync.tools.getLocalizedMessage(isAdding ? "acl.add" : "acl.modify", "dav"));
                                        }
                                    }
                                } else {
                                    tbSync.errorlog.add("warning", syncdata.ownerInfo, "cardnotfoundbutinchangelog::" + changes[i].id);
                                }
                            }
                            ...
                            if (!permissionError[changes[i].status]) { //if this operation failed already, do not retry
                                ...
                                if (response  && response.softerror) {
                                    if (response.softerror != 404) { //we cannot do anything about a 404 on delete, the card has been deleted here and is not avail on server
                                        permissionError[changes[i].status] = true;
                                        tbSync.errorlog.add("warning", syncdata.ownerInfo, "missing-permission::" + tbSync.tools.getLocalizedMessage("acl.delete", "dav"));
                                    }
                                }
                            }
